...
- **Lần đầu chạy**: script hỏi API key → mở app → sidebar **⚙️ Cài đặt → API Key** → copy → dán vào Termux → Enter.
  Key tự lưu vào `~/.velocitix_key`, các lần sau khỏi nhập lại.
- Gõ câu hỏi, Enter để gửi.
- Gõ `/model` bất cứ lúc nào để đổi model đang dùng (Smart / Fast / Lite / Coding — Coding chỉ có tác dụng thật nếu bạn là chủ app, người khác chọn sẽ tự âm thầm đổi về Smart).
...
- Tối đa **3 request/phút**
- Tối đa **200 request/ngày**
- Tạo lại key mới (nếu lỡ lộ key) tối đa **2 lần/ngày** — tự reset sau 24h, không phải giới hạn vĩnh viễn
...
| `[Lỗi 429]` — "Gửi quá nhanh" | Vượt quá 3 request/phút | Chờ khoảng 1 phút rồi gửi lại |
| `[Lỗi 429]` — "hết lượt dùng hôm nay" | Vượt quá 200 request/ngày | Chờ qua ngày hôm sau (giới hạn tự reset) |
...
| `[Lỗi mạng]` | Điện thoại mất mạng hoặc worker chưa deploy | Kiểm tra kết nối mạng, hoặc deploy lại worker |
| Chữ tiếng Việt hiện lỗi (ô vuông/dấu hỏi) | Terminal Termux không đặt locale UTF-8 | Script đã tự ép UTF-8; nếu vẫn lỗi, chạy thêm: `echo 'export LANG=en_US.UTF-8' >> ~/.bashrc` rồi mở lại Termux |
